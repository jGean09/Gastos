...
- [ ] Modificar `saveExpense` em `app.js`. Injetar um objeto temporário com `_fireId: 'temp_' + id` e `_syncStatus: 'pending'` no `AppState.allReceipts`.
- [ ] O modal é fechado na hora e `renderAll()` é disparado para exibir a alteração instantânea.
- [ ] A chamada para a API (`POST` ou `PATCH`) é executada em background.
- [ ] Em caso de SUCESSO: O ID `temp_` é substituído pelo ID real e o `_syncStatus` é deletado.
- [ ] Em caso de FALHA: O item recebe `_syncStatus: 'failed'` e a tela é atualizada.
- [ ] O front-end exibe uma estilização específica (`style.css` `.receipt-failed`) para itens em estado de erro, incluindo um botão de "Tentar Novamente" que chama a função de salvar novamente.
