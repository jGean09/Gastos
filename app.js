...
window.addEventListener('error', function(event) {
  alert("Erro Crítico no Código: " + event.message);
});

window.addEventListener('unhandledrejection', function(event) {
  alert("Erro Crítico na Nuvem/Firebase: " + (event.reason ? event.reason.message : "Desconhecido"));
});
...
window.verificarSenha = function() {
  try {
    ...
    if (!inputEl || !erroMsg) {
      alert("Erro na Interface: Campos de login não encontrados.");
      return;
    }
    ...
    if (!input) {
      ...
      erroMsg.textContent = 'Por favor, digite a senha!';
      return;
    }
...
function liberarAcesso() {
  try {
    ...
    alert("Erro ao carregar os painéis do aplicativo: " + error.message);
  }
}
...
// ── 5. COMUNICAÇÃO COM FIREBASE (BLINDADA) ──
async function loadSettings() {
  try {
    const snap = await getDoc(doc(db, 'config', 'settings'));
    if (snap.exists()) appSettings = { ...appSettings, ...snap.data() };
  } catch(e) { 
    console.warn("Aviso (loadSettings): Falha ao puxar configurações. Usando padrão local.", e.message);
  }
}
...
async function saveSettingsToCloud() {
  ...
  try {
    ...
    setSyncStatus('err'); 
    window.showToast("❌ Falha ao salvar configurações na nuvem.");
  }
}
...
async function loadReceipts() {
  ...
  try {
    ...
    console.error("Erro ao renderizar histórico inicial:", error);
  }
}
...
async function addReceiptToCloud(receipt) {
  ...
  try {
    ...
    setSyncStatus('err');
    window.showToast('❌ Erro ao salvar na nuvem: ' + e.message);
    return false;
  }
}
...
async function deleteReceiptFromCloud(fireId) {
  ...
  try {
    ...
    setSyncStatus('err');
    window.showToast("❌ Falha ao excluir do banco: " + e.message);
    return false;
  }
}
...
window.toggleReceiptStatus = async function(fireId) {
  try {
    ...
    setSyncStatus('err');
    window.showToast('❌ Erro ao atualizar status: ' + e.message);
  }
};
...
async function checkAuthAndBoot() {
  try {
    ...
    alert("Erro na inicialização (Boot): " + error.message);
  }
}
...
async function initApp() {
  try {
    ...
    window.showToast("Erro durante o carregamento inicial: " + error.message);
  }
}

function today() { return new Date().toISOString().split('T')[0]; }

// ── GASTOS E LÓGICA DE NEGÓCIO ──
window.saveQuickExpense = async function() {
  try {
    ...
    const receipt = {
      id: Date.now(), store: 'Lançamento Avulso', date: today(), payer: payer, method: 'Avulso', status: 'open',
    ...
    window.showToast("Erro ao processar lançamento avulso: " + error.message);
  }
};
...
window.clearAllData = async function() {
  ...
  try {
    ...
    setSyncStatus('err'); 
    window.showToast('❌ Erro ao apagar dados: ' + e.message); 
  }
};
...
window.extractWithGemini = async function() {
  try {
    ...
    window.showToast('❌ Erro na extração da IA: ' + err.message);
  } finally {
...
window.saveReceipt = async function() {
  try {
    ...
    window.showToast("Erro ao montar o salvamento: " + error.message);
  }
};
